...
import ru.ifmo.ctddev.rybak.collections.Bag;
...
public class BagTest {
	...
	Class<? extends Collection> classForTest = Bag.class;

	// public BagTest(Class<? extends Collection> classForTest) {
	...
	public <T> Collection<T> newInstance(Collection<? extends T> collection) {
		try {
			for (Constructor<?> constructor : classForTest.getConstructors()) {
				if (constructor.getParameterTypes().length == 1) {
					return (Collection<T>) constructor.newInstance(collection);
				}
			}
	...
	@Test(expected = ConcurrentModificationException.class)
	public void testConcurrentModificationExceptionWithTwoIterators()
			throws Exception {
		bag.add(1);
		bag.add(2);
	...
	@Test
	public void testTwoIteratorsRemove() {
		bag.addAll(Arrays.asList(1, 2, 3, 4, 5, 6, 7, 8));
		Iterator<Integer> it2 = bag.iterator();
		it2.hasNext();
		Iterator<Integer> it1 = bag.iterator();
	...
	public static void main(String[] args) throws ClassNotFoundException {
		// BagTest.classForTest = (Class<? extends Collection>)
		// Class.forName(args[0]);

		org.junit.runner.JUnitCore.main("bag.BagTest");
